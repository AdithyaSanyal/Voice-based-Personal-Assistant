...
api_key = "google-custom-search_api_key"
def home(request):
    return render(request, 'landing_page.html')
def text_retrieval(request):
    inp_value = request.POST.get('text')
    context = {'inp_value': inp_value}
    return inp_value

def text_retrieval_input(request):
    return render(request, 'text_retrieval.html')

def text_retrieval_output(request):
    title = str(text_retrieval(request))
    resource = build("customsearch", 'v1', developerKey=api_key).cse()
    con = resource.list(q=''.join(title), cx='009557628044748784875:5lejfe73wrw').execute()
    content = ""
    for item in con['items']:
        content = content + item['title']+"\t"
        content = content + item['link']
        content = content + "\n"
    context = {'content': content}
    return render(request, 'output_page.html', context)

def image_retrieval(request):
    inp_value = request.POST.get('image')
    context = {'inp_value': inp_value}
    return inp_value

def image_retrieval_input(request):
    return render(request, 'image_retrieval.html')

def image_retrieval_output(request):
    title = str(image_retrieval(request))
    resource = build("customsearch", 'v1', developerKey=api_key).cse()
    con = resource.list(q=''.join(title), cx='009557628044748784875:5lejfe73wrw',searchType='image').execute()
    content = ""
    for item in con['items']:
        content = content + item['title']+"\t"
        content = content + item['link']
...
def text_summarizer_input(request):
    return render(request, 'File_Upload_Page.html')
...
def text_summarizer_output(request):
    file_name=simple_upload(request)
...
def question_generation_input(request):
    return render(request, 'question_generation.html')
    
def question_generation_output(request) :
    
    qg = QuestionGenerator()
    qa_list = qg.generate(str(question_generation(request)), num_questions=6, answer_style='multiple_choice')
    #qg.print_qa(qa_list)
    content=''
    for i in qa_list:
        content=content+'Q: '
        content=content+i['question']+'\n'+'A: '+'\n'
        count=1
        for b in i['answer']:
            content=content+str(count)+'. '
            if(b['correct']):
                content=content+b['answer']+' (Correct)\n'
            else:
                content=content+b['answer']+'\n'
            count=count+1
        content=content+'\n'
    context = {'content': content}
    return render(request, 'output_page.html',context)